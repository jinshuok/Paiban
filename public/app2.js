...
function canEditMember(memberId) {
  ...
  return memberId === getMyMemberId();
}

function getMySelectedCount() {
  const myMemberId = getMyMemberId();
  let count = 0;
  selCells.forEach(key => {
    const cell = document.querySelector(`[data-key="${key}"]`);
    if (cell && cell.dataset.mid === myMemberId) count++;
  });
  return count;
}
...
function renderTable() {
  ...
  members.forEach((m) => {
    ...
    nc.addEventListener('click', () => openMemberProfile(m));
    row.appendChild(nc);
...
function onCellClick(e, cell) {
  ...
  const memberId = cell.dataset.mid;
...
function addToSelection(cell) {
  ...
  const memberId = cell.dataset.mid;
  if (!canEditMember(memberId)) {
    cell.classList.add('selecting-others');
    return;
  }
  selCells.add(key);
...
function showBatchPopup() {
  ...
  const myCount = getMySelectedCount();
  const totalCount = selCells.size;
  if (currentUserRole !== 'admin' && totalCount !== myCount) {
    document.getElementById('batchPopupHeader').textContent = `选中自己的格子 ${myCount} 个`;
  } else {
    document.getElementById('batchPopupHeader').textContent = `已选 ${totalCount} 格`;
  }
...
function batchApply(statusId) {
  ...
  selCells.forEach(key => {
    ...
    if (cell) {
      ...
      if (currentUserRole === 'admin' || mid === myMemberId) {
        applyCellStatus(cell, statusId === '__clear' ? null : statusId);
        applied++;
      }
    }
  });
  const st = statusId === '__clear' ? null : getStatus(statusId);
  showToast(`批量设置：${st ? st.label : '清空'} × ${applied}`);
  clearSelection();
}
...
function updateMobileSheetCount() {
  ...
  const totalCount = selCells.size;
  if (currentUserRole !== 'admin' && totalCount !== myCount) {
    document.getElementById('mobileSheetCount').textContent = `选中自己的格子 ${myCount} 个`;
  } else {
    document.getElementById('mobileSheetCount').textContent = `已选 ${totalCount} 格`;
  }
}
...
function mobileBatchApply(statusId) {
  ...
  selCells.forEach(key => {
    ...
    if (cell) {
      const mid = cell.dataset.mid;
      if (currentUserRole !== 'admin' && mid !== myMemberId) return;
...
function renderModalTab() {
  ...
  if (currentModalTab === 'members') {
    ...
    bindStatusesTab();
  } else if (currentModalTab === 'theme') {
    body.innerHTML = renderThemeTab();
    bindThemeTab();
  ...
  return `
    <div class="space-y-4">
      <div class="text-sm text-slate-700">
        <div class="font-medium mb-2">选择主题</div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <label class="cursor-pointer border rounded-xl p-4 hover:bg-slate-50 transition ${currentTheme === 'Class' ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200'}">
            <div class="flex items-center gap-3">
              <input type="radio" name="themeSelect" value="Class" class="accent-indigo-600" ${currentTheme === 'Class' ? 'checked' : ''}>
              <div>
                <div class="font-semibold text-slate-800">Class</div>
                <div class="text-xs text-slate-500">经典主题，圆角色块，色彩丰富</div>
              </div>
            </div>
          </label>
          <label class="cursor-pointer border rounded-xl p-4 hover:bg-slate-50 transition ${currentTheme === 'Excel' ? 'border-indigo-500 bg-indigo-50' : 'border-slate-200'}">
            <div class="flex items-center gap-3">
              <input type="radio" name="themeSelect" value="Excel" class="accent-indigo-600" ${currentTheme === 'Excel' ? 'checked' : ''}>
              <div>
                <div class="font-semibold text-slate-800">Excel</div>
                <div class="text-xs text-slate-500">表格主题，极简风格，清晰边框</div>
              </div>
            </div>
          </label>
        </div>
    ...
    radio.addEventListener('change', () => {
      if (radio.checked) {
        ...
        if (newTheme !== currentTheme) {
          localStorage.setItem('sched_theme', newTheme);
          if (newTheme === 'Class') {
            window.location.href = 'index.html';
          } else if (newTheme === 'Excel') {
...
//  EDIT PROFILE DRAWER
// ═══════════════════════════════════════════════
function openEditProfileDrawer(member) {
  if (!member) {
    member = CONFIG.members.find(m => m.uid === currentUsername);
  }
  if (!member) {
    showToast('未找到当前成员信息');
    return;
...
function openMemberProfile(member) {
  if (currentUserRole === 'admin' && member.uid !== currentUsername) {
    openAdminEditMemberDrawer(member);
  } else {
    openEditProfileDrawer(member);
  }
}

let _adminEditTargetMember = null;

function openAdminEditMemberDrawer(member) {
  _adminEditTargetMember = member;
  document.getElementById('adminEditMemberName').value = member.name || '';
  document.getElementById('adminEditMemberPhone').value = member.uid || '';
  document.getElementById('adminEditMemberNewPassword').value = '';
  renderAdminEditMemberThirdParties(member.thirdParties || []);
  document.getElementById('adminEditMemberModal').classList.remove('hidden');
  document.getElementById('adminEditMemberDrawer').classList.remove('translate-x-full');
}

function closeAdminEditMemberDrawer() {
  document.getElementById('adminEditMemberModal').classList.add('hidden');
  document.getElementById('adminEditMemberDrawer').classList.add('translate-x-full');
  _adminEditTargetMember = null;
}

function renderAdminEditMemberThirdParties(list) {
  const container = document.getElementById('adminEditMemberThirdParties');
...
function getAdminEditMemberThirdParties() {
  const arr = [];
  document.querySelectorAll('#adminEditMemberThirdParties .third-party-row').forEach(row => {
...
async function saveAdminEditMember() {
  if (!_adminEditTargetMember) return;
  const name = document.getElementById('adminEditMemberName').value.trim();
  const phone = document.getElementById('adminEditMemberPhone').value.trim();
  const newPassword = document.getElementById('adminEditMemberNewPassword').value.trim();
  const thirdParties = getAdminEditMemberThirdParties();
  if (!name || !phone) {
  ...
  try {
    const res = await fetch('/api/admin/member-profile', {
      method: 'POST',
      headers: apiHeaders({ 'Content-Type': 'application/json' }),
      body: JSON.stringify({ memberId: _adminEditTargetMember.id, name, uid: phone, thirdParties, newPassword: newPassword || undefined })
    });
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
    ...
    const data = await res.json();
    _adminEditTargetMember.name = name;
    _adminEditTargetMember.uid = phone;
    _adminEditTargetMember.thirdParties = thirdParties;
    try { localStorage.setItem('sched_config_v2', JSON.stringify(CONFIG)); } catch(e){}
    showToast('保存成功');
    closeAdminEditMemberDrawer();
    render();
  } catch (e) {
    showToast('网络错误');
  }
...
async function saveEditProfile() {
  ...
  try {
    const res = await fetch('/api/auth/profile', {
      method: 'POST',
      headers: apiHeaders({ 'Content-Type': 'application/json' }),
      body: JSON.stringify({ name, uid: phone, thirdParties, newPassword: newPassword || undefined })
    });
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
      showToast(data.error || '保存失败');
...
// Settings guard
document.getElementById('adminEditMemberOverlay')?.addEventListener('click', closeAdminEditMemberDrawer);
document.getElementById('adminEditMemberClose')?.addEventListener('click', closeAdminEditMemberDrawer);
document.getElementById('adminEditMemberCancel')?.addEventListener('click', closeAdminEditMemberDrawer);
document.getElementById('adminEditMemberSave')?.addEventListener('click', saveAdminEditMember);
document.getElementById('adminEditMemberAddThirdParty')?.addEventListener('click', () => {
  const container = document.getElementById('adminEditMemberThirdParties');
  const row = document.createElement('div');
  row.className = 'flex items-center gap-2 third-party-row';
  row.innerHTML = `
...
document.getElementById('settingsBtn')?.addEventListener('click', () => {
  ...
  document.querySelectorAll('.modal-tab').forEach(t => {
    ...
    if (tabName === 'theme' || isAdmin) {
      t.style.display = '';
    } else {
      t.style.display = 'none';
    }
  });
  openSettings(isAdmin ? 'members' : 'theme');
});
